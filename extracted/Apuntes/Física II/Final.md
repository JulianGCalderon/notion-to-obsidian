...
$$
\text{Integral}: \oiint \vec Ed\vec s = \frac{Q_\text{enc}}{\varepsilon_0}
$$
...
&\overbrace
{\oiint\limits_{S} \vec D \cdot   d \vec s = Q_{\text{enc}}^{\text{libres}}
...
&\oiint\limits_{S} \vec P \cdot   d \vec s = -Q_{\text{enc}}^{\text{pol}} \implies \vec\nabla \vec P = -\delta^{\text{pol}}
...
!![[Apuntes/Física II/attachments/Final 1.png]]
